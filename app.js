...
function recoverAllData() {
    ...
    let recoveredFinance = false;
    let recoveredInventory = false;
    let foundData = [];
    ...
    allKeys.forEach(key => {
        try {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
                ...
                if (firstItem.date && (firstItem.amount !== undefined || firstItem.type)) {
                    // Looks like expense data
                    foundData.push({
                        key: key,
                        type: 'finance',
                        count: parsed.length,
                    ...
                    // If it's not the current key, try to recover it
                    if (key !== 'nutritionExpenses' && tracker) {
                        console.log(`Found finance data in key: ${key} with ${parsed.length} entries`);
                        if (confirm(`Found ${parsed.length} finance entries in key "${key}". Restore them?`)) {
                            tracker.expenses = parsed;
                            tracker.saveExpenses();
                            tracker.renderExpenses();
                            tracker.updateCharts();
                            recoveredFinance = true;
                            alert(`✅ Recovered ${parsed.length} finance entries!`);
                        }
                    }
                }
                
                // Check if it looks like inventory data
                if (firstItem.productId || firstItem.type === 'summary' || firstItem.type === 'product') {
                    foundData.push({
                        key: key,
                        type: 'inventory',
                        count: parsed.length,
                    ...
                    if (inventoryManager) {
                        // Check which type of inventory data
                        if (key.includes('StockIn') || (firstItem.productId && firstItem.quantity && firstItem.date)) {
                            console.log(`Found Stock In data in key: ${key} with ${parsed.length} entries`);
                            if (confirm(`Found ${parsed.length} Stock In entries in key "${key}". Restore them?`)) {
                                inventoryManager.stockInHistory = parsed;
                                inventoryManager.saveStockInHistory();
                                inventoryManager.renderStockInHistory();
                                recoveredInventory = true;
                                alert(`✅ Recovered ${parsed.length} Stock In entries!`);
                            }
                        } else if (key.includes('DailyUsage') || firstItem.type === 'summary' || firstItem.type === 'product') {
                            console.log(`Found Daily Usage data in key: ${key} with ${parsed.length} entries`);
                            if (confirm(`Found ${parsed.length} Daily Usage entries in key "${key}". Restore them?`)) {
                                inventoryManager.dailyUsage = parsed;
                                inventoryManager.saveDailyUsage();
                                inventoryManager.renderDailyUsage();
                                recoveredInventory = true;
                                alert(`✅ Recovered ${parsed.length} Daily Usage entries!`);
                            }
                        }
                    }
                }
            ...
            // Check if it looks like stock data (object with product IDs)
            if (typeof parsed === 'object' && !Array.isArray(parsed)) {
                const keys = Object.keys(parsed);
                if (keys.length > 0 && parsed[keys[0]] && parsed[keys[0]].quantity !== undefined) {
                    foundData.push({
                        key: key,
                        type: 'stock',
                        count: keys.length,
                        sample: { productId: keys[0], data: parsed[keys[0]] }
                    });
                    
                    if (key !== 'inventoryStock' && inventoryManager) {
                        console.log(`Found stock data in key: ${key} with ${keys.length} products`);
                        if (confirm(`Found stock data for ${keys.length} products in key "${key}". Restore them?`)) {
                            // Merge with existing
                            Object.keys(parsed).forEach(productId => {
                                if (!inventoryManager.stockData[productId] || inventoryManager.stockData[productId].quantity === 0) {
                                    inventoryManager.stockData[productId] = parsed[productId];
                                }
                            });
                            inventoryManager.saveStockData();
                            inventoryManager.renderCurrentStock();
                            recoveredInventory = true;
                            alert(`✅ Recovered stock data for ${keys.length} products!`);
                        }
                    }
                }
            }
    ...
    // Show summary
    let message = '=== DATA RECOVERY SUMMARY ===\n\n';
    message += `Found ${foundData.length} potential data sources:\n\n`;
    
    foundData.forEach(item => {
        message += `- ${item.type.toUpperCase()}: ${item.count} items in key "${item.key}"\n`;
    });
    
    if (recoveredFinance || recoveredInventory) {
        message += '\n✅ Some data was recovered!';
        if (recoveredFinance) message += '\n✅ Finance data restored';
        if (recoveredInventory) message += '\n✅ Inventory data restored';
    } else {
        message += '\n⚠️ No data was automatically recovered.';
    ...
    // Reload page to refresh displays
    if (recoveredFinance || recoveredInventory) {
        if (confirm('Data recovered! Reload page to see all changes?')) {
            location.reload();
        }
    }
}
